...
char exception2[3][6]={"SVC", "CLEAR", "TIXR"};
...
Hash* findBucket(char* str){  // findBucket 함수 : 넘겨받은 op instruction이 담긴 bucket을 찾아냄
	...
	while(ret){  // 불러온 list를 하나하나 뒤져봄
		if(!strcmp(((Hash*)ret)->opInst, str)){
			return ret;
		}
...
void CreateTable(){  // CreateTable 함수 : opcode.txt 파일을 바탕으로 하여 hash table을 생성함
	...
	while(fscanf(fp, "%s %s %s", opCode, opInst, opMod)==3){  // 해당 txt파일에는 각 instruction별로 3개의 정보가 있기에, 총 3개의 문자열을 입력받음
		...
		((Hash*)now)->next=NULL;
		((Hash*)now)->operand=operandChk((Hash*)now);
...
int operandChk(Hash* tmp){
	...
	switch(tmp->mod){
		...
		case 1:
			...
			for(int i=0; i<3; i++){
				if(!strcmp(tmp->opInst, exception2[i]))
					ret=1;
			}
...
int hashfunction(char* str){  // hashfunction 함수 : 주어진 정보를 토대로 해싱 값을 생성함
	...
	return abs(ret)%20;  // 임의로 수식을 정해서 총 20개의 list에 나뉘어 담기도록 함
}
